...
def registration_view(request):
    if request.method == 'POST':
        ...
        #     return Response(data)
        #
        # kra_pin = request.data.get('kra_pin', '0')
        # if validate_kra_pin(kra_pin) is not None:
        #     data['error_message'] = 'Same Kra pin is already in use.'
        #     data['response'] = 'Error'
        #     return Response(data)
        #
        # id_no = request.data.get('id_no', '0')
        # if validate_id_no(id_no) is not None:
        #     data['error_message'] = 'provided ID number is already in use.'
        #     data['response'] = 'Error'
        #     return Response(data)
        #
        # phone = request.data.get('phone', '0')
        # if validate_phone(phone) is not None:
        #     data['error_message'] = 'The number is already in use.'
        #     data['response'] = 'Error'
        #     return Response(data)
        ...
        if serializer.is_valid():
            ...
            data['kra_pin'] = account.kra_pin
            data['id_no'] = account.id_no
...
def validate_kra_pin(kra_pin):
    account = None
    try:
        account = Account.objects.get(kra_pin=kra_pin)
    except Account.DoesNotExist:
        return None
    if account is not None:
        return kra_pin
...
def validate_id_no(id_no):
    account = None
    try:
        account = Account.objects.get(id_no=id_no)
    except Account.DoesNotExist:
        return None
    if account is not None:
        return id_no
